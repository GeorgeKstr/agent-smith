...
import { setMeta } from "../db/db.js";
import { scanProjectFiles } from "./scanner.js";
import { hashFile } from "./hashing.js";
import { extractImports, extractSymbols, isParseableLanguage } from "./extractor.js";
import { rebuildImports } from "./graph.js";
import { isProviderAvailable, listProviderModels, resolveProviderModelName, generateWithProvider } from "../providers/providers.js";
...
export function createIndexer({ root, config, db, events, enableIntel = true }: IndexerArgs) {
  ...
  async function quickStartupScan() {
    ...
    if (config.index.fileCards) {
      for (const item of changedParseable) {
        try {
          const fileRow = db.prepare("SELECT hash, summary FROM files WHERE id = ?").get(item.fileId) as
            { hash: string; summary: string | null } | undefined;
          if (!fileRow) continue;
          const dbSymbols = db.prepare(
            "SELECT name, kind, start_line, end_line, signature FROM symbols WHERE file_id = ?"
          ).all(item.fileId) as Array<{ name: string; kind: string; start_line: number; end_line: number; signature: string | null }>;
          ...
          const dbImports = db.prepare(
            "SELECT import_text, COALESCE(resolved_path, import_text) as specifier FROM imports WHERE from_file_id = ?"
          ).all(item.fileId) as Array<{ import_text: string; specifier: string }>;
      ...
      while (summaryQueue.length > 0) {
        ...
        if (config.index.fileCards) {
          try {
            const dbSymbols = db.prepare(
              "SELECT name, kind, start_line, end_line, signature FROM symbols WHERE file_id = ?"
            ).all(fileRow.id) as Array<{ name: string; kind: string; start_line: number; end_line: number; signature: string | null }>;
            const symbols = dbSymbols.map((s) => ({
              name: s.name, kind: s.kind, startLine: s.start_line, endLine: s.end_line, signature: s.signature ?? undefined,
            }));
            const dbImports = db.prepare(
              "SELECT import_text, COALESCE(resolved_path, specifier) as specifier FROM imports WHERE from_file_id = ?"
            ).all(fileRow.id) as Array<{ import_text: string; specifier: string }>;
            const imports = dbImports.map((i) => ({ importText: i.import_text, specifier: i.specifier }));
            const card = buildHeuristicFileCard({
              path: relPath,
              content,
