...
import type { ExtractedImport } from "../types/index.js";
 ...
 */
export function expandGraph(db: SmithDatabase, seedFileIds: number[], depth: number): Set<number> {
  const visited = new Set<number>(seedFileIds);
  let frontier = new Set<number>(seedFileIds);
  ...
  for (let level = 0; level < depth; level++) {
    ...
    for (const fileId of frontier) {
      for (const row of [...outgoing.all(fileId), ...incoming.all(fileId)] as Array<{ id: number }>) {
...
export function fileNeighbors(db: SmithDatabase, relPath: string): GraphNeighbors | undefined {
  const file = db.prepare("SELECT id FROM files WHERE path = ?").get(relPath) as { id: number } | undefined;
  if (!file) return undefined;

  const imports = (
    db
      .prepare(
        "SELECT f.path AS path FROM imports i JOIN files f ON f.id = i.to_file_id WHERE i.from_file_id = ? AND i.to_file_id IS NOT NULL"
      )
...
export function renderFileGraph(neighbors: GraphNeighbors): string {
  const lines: string[] = [];
  lines.push(`           ┌─────────────────────────────┐`);
  ...
  if (neighbors.imports.length === 0) lines.push("     · (none / external only)");
  for (const dep of neighbors.imports.slice(0, 20)) lines.push(`     ├─▶ ${dep}`);
  lines.push("");
  lines.push(`  imported by (${neighbors.importedBy.length}) ◀──`);
  if (neighbors.importedBy.length === 0) lines.push("     · (entry point / unused)");
  for (const dep of neighbors.importedBy.slice(0, 20)) lines.push(`     ◀─┤ ${dep}`);
  return lines.join("\n");
}
