 ...
 */
export const GLOBAL_TAGS: ReadonlyArray<{ id: number; name: string; description: string }> = [
...
export function seedTags(db: Database.Database): void {
  ...
  const tx = db.transaction(() => {
    for (const tag of GLOBAL_TAGS) upsert.run(tag);
  });
  tx();
}

/** Render the global tag map as a compact list for prompting. */
export function tagMapForPrompt(): string {
 ...
 */
export function heuristicTags(relativePath: string, content: string): number[] {
  const haystack = `${relativePath}\n${content.slice(0, 4000)}`.toLowerCase();
  const hits = new Set<number>();

  const signals: Array<[number, RegExp]> = [
  ...
  for (const [id, pattern] of signals) {
    if (pattern.test(haystack)) hits.add(id);
  }
