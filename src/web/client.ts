...
async function loadSessions(){try{S.chat.sessions=(await aapi('/chat/sessions').catch(function(){return{sessions:[]};})).sessions||[];}catch(e){S.chat.sessions=[];}if(!S.chat.sid&&S.chat.sessions.length===0){try{var r=await aapi('/chat/sessions',{method:'POST',body:{}});if(r.session){S.chat.sessions=[r.session];S.chat.sid=r.session.id;await loadMsgs(r.session.id);if(!S.chat.allModels.length)await loadModels();}}catch(e){}}else if(!S.chat.sid&&S.chat.sessions.length>0){S.chat.sid=S.chat.sessions[0].id;await loadMsgs(S.chat.sessions[0].id);}else renderMainOnly();}
async function loadMsgs(sid){if(!sid)return;try{var r=await aapi('/chat/sessions/'+sid);S.chat.msgs=r.messages||[];S.chat.qtns=r.openQuestions||[];}catch(e){S.chat.msgs=[];S.chat.qtns=[];}for(var i=S.chat.msgs.length-1;i>=0;i--){var m=S.chat.msgs[i];if(m.model){S.chat.model=m.model;break;}}renderMainOnly();scrollChat();}
