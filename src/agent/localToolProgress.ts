export type LocalToolProgress = {
  consecutiveSearches: number;
  consecutiveReads: number;
  consecutiveInvalidOutputs: number;
  totalSearches: number;
  searchesAfterFirstRead: number;
  firstReadHappened: boolean;
  lastToolName?: string;
  lastSearchQueries: string[];
  lastReadPaths: string[];
  searchedSinceLastRead: boolean;
  readSinceLastSearch: boolean;
};

export function createEmptyLocalToolProgress(): LocalToolProgress {
  return {
...
export function updateLocalToolProgress(input: {
  ...
  if (input.toolName === "search") {
    ...
    if (query) next.lastSearchQueries = [...next.lastSearchQueries, query].slice(-5);
    ...
    if (path) next.lastReadPaths = [...next.lastReadPaths, path].slice(-5);
  } else {
    next.consecutiveSearches = 0;
    next.consecutiveReads = 0;
  }
