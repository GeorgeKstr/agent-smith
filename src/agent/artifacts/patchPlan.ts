export type PatchPlan = {
  ...
  edits: Array<{
    ...
    change: string;
    reason: string;
    preferredTool: "edit" | "replace_lines" | "create_file";
  }>;
  checks: string[];
...
export function emptyPatchPlan(goal: string): PatchPlan {
  return {
    goal,
    edits: [],
    checks: [],
...
export function buildPatchPlan(input: {
  ...
  return {
    ...
    checks: [],
    risk: riskLevel as PatchPlan["risk"],
    needsUserInput: false,
    questions: [],
  };
}

export function renderPatchPlanForApply(plan: PatchPlan): string {
  if (plan.edits.length === 0) return "No edits in plan.";
