...
export function getPatchPhase(input: {
  ...
  const searched = evidence.toolResults.some(
    (result) => result.tool === "search" && result.ok
  );

  if (searched) return "inspecting";
