...
export async function compactToolResult(input: {
  ...
  switch (toolName) {
    case "read": {
      ...
      const remainingUnknowns = memory.remainingUnknowns.filter(
        (u) => !u.toLowerCase().includes(path.toLowerCase())
      );
    ...
    case "replace_lines": {
      ...
      return mergeWorkingMemory(memory, {
        ...
        remainingUnknowns: memory.remainingUnknowns.filter(
          (u) => !u.toLowerCase().includes(path.toLowerCase())
        ),
      });
    }
    ...
    case "check": {
      return mergeWorkingMemory(memory, {
        ...
        remainingUnknowns: rawResult.ok
          ? memory.remainingUnknowns.filter(
              (u) => !u.toLowerCase().includes("check") && !u.toLowerCase().includes("verify")
            )
