import {
  ...
  runIntentScenarios, runParserTests,
} from "./trajectoryHarness.js";
import type { TrajectoryScenario } from "./trajectoryHarness.js";
...
import type { FocusedAgentStatus } from "./focusedAgentLoop.js";
...
const intentScenarios: TrajectoryScenario[] = [
...
test("phase tools subset of profile tools (except 'final')", () => {
  const phases = ["chat", "ask", "explore", "plan_patch", "apply_patch", "verify"] as const;
  for (const phase of phases) {
    ...
    for (const t of pts) {
      if (t === "final") continue;
      assert(ptools.includes(t),
        `${phase} tool "${t}" should be in "${profile}" profile [${ptools.join(", ")}]`);
    }
  }
});
...
test("FocusedAgentStatus includes 'blocked'", () => {
  const statuses: FocusedAgentStatus[] = ["completed", "partial", "failed", "waiting_for_user", "blocked"];
