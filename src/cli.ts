...
export function createCli() {
  ...
  function applyOrganizerOpts(config: SmithConfig, opts: { url?: string; token?: string; name?: string; id?: string }) {
    if (opts.url) config.organizer.url = opts.url;
    if (opts.token) config.organizer.token = opts.token;
    if (opts.name) config.organizer.agentName = opts.name;
    if (opts.id) config.organizer.agentId = opts.id;
  }
  ...
  organizerCmd
    ...
    .option("--json", "output JSON")
    .action(async (opts: { url?: string; token?: string; apiUrl?: string; name?: string; id?: string; json: boolean }) => {
      const { root, config, db } = await bootstrap();
  ...
  organizerCmd
    ...
    .action(async (opts: { url?: string; token?: string; apiUrl?: string; name?: string; id?: string; interval?: string }) => {
      const { root, config, db } = await bootstrap();
      applyOrganizerOpts(config, opts);
      if (opts.interval) config.organizer.heartbeatMs = Number(opts.interval);
