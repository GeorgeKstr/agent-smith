...
export async function applyAcceptedChangeSet(args: ApplyArgs): Promise<ApplyResult> {
  ...
  const acceptedPaths: string[] = cs.status === "accepted"
...
export async function validateAcceptedHunksDiff(args: {
  ...
  db: SmithDatabase;
  changeSetId: string;
}): Promise<{ ok: boolean; message: string; diff: string; files: string[] }> {
