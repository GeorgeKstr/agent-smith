...
import type { ParsedDiffFile } from "./diffParser.js";

type AcceptedHunkRef = { path: string; hunkIndex: number };

export function buildAcceptedHunksDiff(args: {
  originalDiff: string;
  acceptedHunks: AcceptedHunkRef[];
}): string {
  const { originalDiff, acceptedHunks } = args;
  if (!originalDiff || !originalDiff.trim()) return "";
  if (acceptedHunks.length === 0) return "";

  const parsed = parseUnifiedDiff(originalDiff);

  const acceptedByFile = new Map<string, Set<number>>();
