...
import type { SmithConfig, TaskClassification, ScoredFile, RetrievalReason } from "../types/index.js";
...
async function ripgrepHits(root: string, keywords: string[], ignore: string[]): Promise<Map<string, number>> {
  ...
  for (const dir of ignore) args.push("--glob", `!${dir}/**`);
  ...
  try {
    ...
    for (const line of stdout.split("\n")) {
      ...
      if (file && Number.isFinite(count)) hits.set(file, count);
    }
  } catch {
...
export async function retrieve(args: {
  ...
  const scores = new Map<number, ScoredFile>();

  const files = db.prepare("SELECT id, path, language, is_test, summary FROM files").all() as Array<{
    id: number;
    path: string;
    language: string;
    is_test: number;
  ...
  if (classification.tagIds.length > 0) {
    ...
    const tagRows = db
      ...
      .all(...classification.tagIds) as Array<{ file_id: number; overlap: number }>;
  ...
  const symbols = db.prepare("SELECT file_id, name, kind, start_line, end_line FROM symbols").all() as Array<{
    file_id: number;
    name: string;
    kind: string;
    start_line: number;
  ...
  const seedSymbols: RetrievalResult["seedSymbols"] = [];
  for (const sym of symbols) {
    ...
    if (symbolTargets.some((t) => t.length >= 3 && lname.includes(t))) {
      ...
      if (f) seedSymbols.push({ name: sym.name, path: f.path, kind: sym.kind, startLine: sym.start_line, endLine: sym.end_line });
    }
  }
  ...
  if (config.context.includeTests) {
    for (const entry of [...scores.values()]) {
      if (entry.score <= 0 || entry.isTest) continue;
...
function memoryFileBoost(db: SmithDatabase, keywords: string[]): number[] {
  ...
  const rows = db
    ...
    .all(...ids) as Array<{ file_id: number }>;
...
function dedupeSymbols(symbols: RetrievalResult["seedSymbols"]): RetrievalResult["seedSymbols"] {
  const seen = new Set<string>();
  const out: RetrievalResult["seedSymbols"] = [];
