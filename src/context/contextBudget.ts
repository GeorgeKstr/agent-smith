...
export function budgetForTokenLimit(maxPromptTokens: number): ContextBudget {
  if (maxPromptTokens <= 4096) return { ...DEFAULT_4K_BUDGET, maxPromptTokens };
  return { ...DEFAULT_8K_BUDGET, maxPromptTokens };
}

export function defaultBudgetFromTokens(maxPromptTokens: number): ContextBudget {
