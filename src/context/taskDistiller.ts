...
export async function distillTaskPacket(input: {
  ...
  try {
    const systemParts: string[] = [DISTILL_PROMPT];
    ...
    if (!result.ok || !result.text) return heuristic;

    const parsed = extractJson<unknown>(result.text.trim());
    ...
    if (!validation.success) return heuristic;
    ...
    return {
      ...
      successCriteria:
        data.successCriteria.length > 0 ? data.successCriteria : heuristic.successCriteria,
      nonGoals:
        data.nonGoals.length > 0
      ...
      verificationPlan:
        data.verificationPlan.length > 0 ? data.verificationPlan : heuristic.verificationPlan,
