...
export function renderRetrievalLeads(leads: RetrievalLead[], maxEvidenceChars?: number): string {
  if (leads.length === 0) return "No relevant leads found.";

  const limit = maxEvidenceChars ?? MAX_EVIDENCE_TEXT_CHARS;
  const lines: string[] = ["Relevant leads:"];
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    if (lead.evidence.length > 0) {
      ...
      for (const ev of lead.evidence) {
        const loc =
          ev.lines && ev.lines.length === 2
            ? ` lines ${ev.lines[0]}-${ev.lines[1]}`
            : "";
        const text =
          ev.text.length > limit
