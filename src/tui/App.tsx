...
export function App({ root, config, db, events, indexer }: AppProps) {
  ...
  const [selHunks, setSelHunks] = useState<ChangedHunkReview[]>([])
  const [selHunkIndex, setSelHunkIndex] = useState(0)
  const [reviewFocus, setReviewFocus] = useState<"files" | "hunks">("files")
  ...
  const organizerHeartbeatRef = useRef<{ stop(): void } | null>(null)
  ...
  const [activeQuestion, setActiveQuestion] = useState<{ question: string; options: string[]; selectedIndex: number; command: string | null; source: "slash_command" | "database"; questionId?: string } | null>(null)
  ...
  const handleSlashCommand = useCallback(async (input: string): Promise<boolean> => {
    ...
    switch (cmd) {
      ...
      case "/model-pick": {
        ...
        if (!selected || !selected.includes(":")) return true
        setModelOverride(selected)
        appendOutput(`✓ Model set to ${selected}`)
        return true
      }
      ...
      case "/changes": {
        ...
        const lines: string[] = ["— CHANGE SETS —"]
        for (const cs of all) {
      ...
      case "/change": {
        const id = rawParts[1]
        if (!id) { appendOutput("Usage: /change <id>"); return true }
        const cs = getChangeSet(db, id)
        if (!cs) { appendOutput(`Change set not found: ${id}`); return true }
      ...
      case "/accept": {
        const id = rawParts[1]
        if (!id) { appendOutput("Usage: /accept <id> [<path>]"); return true }
      ...
      case "/reject": {
        const id = rawParts[1]
        if (!id) { appendOutput("Usage: /reject <id> [<path>]"); return true }
        const filePath = rawParts[2]
        if (filePath) {
      ...
      case "/apply": {
        const id = rawParts[1]
        if (!id) { appendOutput("Usage: /apply <id>"); return true }
        const result = await applyAcceptedChangeSet({ root, db, indexer, changeSetId: id })
        const lines: string[] = [result.ok ? "✓" : "✗", result.message]
        if (result.files.length) lines.push(`files: ${result.files.join(", ")}`)
        if (result.checkpointId) lines.push(`checkpoint: ${result.checkpointId}`)
        appendOutput(lines.join(" "))
        return true
      }
      case "/revert": {
        const id = rawParts[1]
        if (!id) { appendOutput("Usage: /revert <id>"); return true }
        const cs = getChangeSet(db, id)
        if (!cs) { appendOutput(`Change set not found: ${id}`); return true }
        if (!cs.checkpointId) {
      ...
      case "/session": {
        ...
        }
        {
          ...
          for (let i = msgs.length - 1; i >= 0; i--) {
            const m = msgs[i]
            if (m.model) { setModelOverride(m.model); break }
          }
          setOutput([])
          setAnswer("")
          setPatchText("")
  ...
  if (view === "changes") {
    ...
    return (
      <ChangesScreen
        changeSet={selChangeSet}
        files={selChangedFiles}
        selectedPath={selFilePath}
        diffPreview={selDiffPreview}
        hunks={selHunks}
        selectedHunkIndex={selHunkIndex}
        ...
        onAcceptAll={async () => {
          ...
          if (cs) { reloadFilesAndHunks(); appendOutput(`Accepted change set ${selChangeId}`) }
        }}
        onRejectAll={async () => {
          const cs = updateChangeSetStatus(db, selChangeId!, "rejected")
          if (cs) { reloadFilesAndHunks(); appendOutput(`Rejected change set ${selChangeId}`) }
        }}
        onApply={async () => {
          ...
          if (cs) { reloadFilesAndHunks() }
        }}
        onFocusChange={(focus) => setReviewFocus(focus)}
        onSelectHunk={(index) => setSelHunkIndex(index)}
        onAcceptHunk={() => {
