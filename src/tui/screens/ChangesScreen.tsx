...
export type ChangesScreenProps = {
  ...
  onRejectAll: () => void
  onApply: () => void
  onFocusChange: (focus: "files" | "hunks") => void
  onSelectHunk: (index: number) => void
  onAcceptHunk: () => void
  onRejectHunk: () => void
  onAcceptFileHunks: () => void
  onRejectFileHunks: () => void
}
...
export function ChangesScreen(props: ChangesScreenProps) {
  useInput((_char, key) => {
    ...
    if (key.upArrow) {
      ...
      if (idx > 0) props.onSelectFile(props.files[idx - 1].path)
      return
    }
    if (key.downArrow) {
      const idx = props.files.findIndex((f) => f.path === props.selectedPath)
      if (idx >= 0 && idx < props.files.length - 1) props.onSelectFile(props.files[idx + 1].path)
      return
    }
    if (_char === "a" && !key.ctrl && !key.meta) {
      props.onAcceptFile()
