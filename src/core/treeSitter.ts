...
import type { ExtractedImport, ExtractedSymbol, SymbolKind } from "../types/index.js";
...
function hashSlice(slice: string): string {
...
function matchBraceBlock(content: string, from: number): number {
...
function skipParens(content: string, from: number): number {
  const open = content.indexOf("(", from);
 ...
 */
function functionBodyEnd(content: string, headerIndex: number): number {
...
function looksLikeJsx(slice: string): boolean {
...
function extractTsSymbols(content: string): ExtractedSymbol[] {
  ...
  for (let m; (m = fnRe.exec(content)); ) {
    ...
    const slice = content.slice(start, end);
    const kind: SymbolKind = isPascalCase(name) && looksLikeJsx(slice) ? "component" : "function";
...
function extractClassMethods(
  ...
  classEnd: number,
  className: string,
  push: (name: string, kind: SymbolKind, start: number, end: number) => void
): void {
...
function findExpressionEnd(content: string, from: number): number {
...
function findStatementEnd(content: string, from: number): number {
  let depth = 0;
...
function extractPythonSymbols(content: string): ExtractedSymbol[] {
  ...
  const stack: Array<{ name: string; indent: number; kind: SymbolKind; start: number; lineNo: number }> = [];

  const indentOf = (line: string) => line.length - line.trimStart().length;

  const flush = (downTo: number, endLineNo: number, endOffset: number) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed && !trimmed.startsWith("#")) {
      ...
      if (fn) {
        const kind: SymbolKind = stack.some((s) => s.kind === "class") ? "method" : "function";
...
function extractTsImports(content: string): ExtractedImport[] {
  ...
  const seen = new Set<string>();
  const add = (importText: string, specifier: string) => {
  ...
  for (const re of patterns) {
    for (let m; (m = re.exec(content)); ) add(m[0], m[1]);
  }
  return imports;
}
