...
import { ensureConfig, loadConfig } from "./config/config.js";
...
import { createTaskPlanStep, listTaskPlanSteps, updateTaskPlanStepStatus, updateTaskPlanStepNotes, deleteTaskPlanStep, replaceTaskPlan, markPlanProgressForPhase } from "./tasks/taskPlanStore.js";
...
async function bootstrap(rootOverride) {
...
export function createCli() {
    ...
    program
        ...
        for (const check of outcome.checks ?? []) {
            ...
            if (!check.ok)
                console.log((check.stderr || check.stdout).slice(0, 1500));
        }
    ...
    program
        ...
        const plan = data?.promptPlan;
        const files = (data?.files ?? []);
        const seedSymbols = (data?.seedSymbols ?? []);
        if (plan?.objective)
    ...
    program
        ...
        if (result.packet) {
            ...
            if (result.packet.prompt.length > 1200)
                console.log("...");
        }
        db.close();
    });
    ...
    taskCmd
        ...
        .argument("<taskId>")
        .argument("<title...>")
        .action(async (taskId, titleParts) => {
        const { db } = await bootstrap();
        const title = titleParts.join(" ");
    ...
    taskCmd
        ...
        const { db } = await bootstrap();
        const step = updateTaskPlanStepStatus(db, stepId, status);
        if (step) {
    ...
    taskCmd
        ...
        .argument("<stepId>")
        .argument("<notes...>")
        .action(async (stepId, notesParts) => {
        const { db } = await bootstrap();
    ...
    taskCmd
        ...
        if (opts.file) {
            try {
                text = await fs.readFile(path.join(root, opts.file), "utf8");
            }
            catch {
                console.log(`Failed to read: ${opts.file}`);
                db.close();
                return;
            }
        }
        if (!text.trim()) {
            console.log("No input. Use --file or --text.");
            db.close();
            return;
        }
    ...
    changesCmd
        ...
        .action(async (opts) => {
        const { db } = await bootstrap();
        const items = listChangeSets(db, { status: opts.status });
        if (opts.json) {
            console.log(JSON.stringify(items, null, 2));
        }
        else if (items.length === 0) {
    ...
    changesCmd
        ...
        .option("--json", "output JSON")
        .action(async (id, pathFilter, opts) => {
        const { db } = await bootstrap();
        ...
        if (opts.json) {
            ...
            if (operations.length === 0) {
                ...
                for (const op of operations) {
                    ...
                    if (op.diff)
                        console.log(`  Diff: ${op.diff.slice(0, 300)}${op.diff.length > 300 ? "..." : ""}`);
                    console.log("");
                }
            }
        }
    ...
    program
        ...
        .action(async (target) => {
        const { db } = await bootstrap();
        const file = db.prepare("SELECT id, path, language, summary FROM files WHERE path = ?").get(target);
        if (file) {
            const symbols = db
            ...
            const tags = db
                ...
                .all(file.id).map((r) => tagName(r.tag_id));
            console.log(`FILE  ${file.path} [${file.language}]`);
            console.log(`TAGS  ${tags.join(", ") || "(none)"}`);
            console.log(`SUMMARY  ${file.summary ?? "(pending)"}`);
            console.log(`SYMBOLS (${symbols.length}):`);
            for (const s of symbols)
                console.log(`  ${s.kind.padEnd(10)} ${s.name}  (L${s.start_line}-${s.end_line})`);
        }
        else {
            const matches = db
                .prepare("SELECT s.name, s.kind, s.start_line, s.end_line, f.path FROM symbols s JOIN files f ON f.id = s.file_id WHERE s.name LIKE ? ORDER BY s.name LIMIT 25")
                .all(`%${target}%`);
            if (matches.length === 0) {
                ...
                for (const m of matches)
                    console.log(`  ${m.kind.padEnd(10)} ${m.name}  ${m.path}:${m.start_line}`);
            }
        }
        db.close();
    });
    program
        ...
        const { db } = await bootstrap();
        let relPath = target;
        const direct = db.prepare("SELECT path FROM files WHERE path = ?").get(target);
        if (!direct) {
            const sym = db
