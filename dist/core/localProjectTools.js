...
import { fileNeighbors } from "./graph.js";
...
export function createLocalProjectTools(args) {
    const { root, db } = args;
    return [
        {
            function: {
        ...
        },
        {
            ...
            handler: ({ query, limit }) => {
                const q = `%${str(query)}%`;
                return db
                    .prepare(`SELECT s.name, s.kind, s.start_line AS startLine, s.end_line AS endLine, f.path
             FROM symbols s
