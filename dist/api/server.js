...
export async function startApiServer(args) {
    ...
    const deps = { root, config, db, events, indexer, runtime, token };
        ...
        try {
            ...
            if (method === "POST" && url === "/api/actions") {
                ...
                if (!body || typeof body !== "object" || typeof body.kind !== "string") {
                    sendJson(res, 400, { ok: false, error: "body.kind must be a string" });
                    return;
                }
                const result = await runtime.dispatch(body);
            ...
            if (url === "/api/events") {
                ...
                req.on("close", () => { for (const [n, h] of handlers)
                    events.off(n, h); });
                return;
            }
            // Task endpoints
            ...
            if (taskId && url === `/api/tasks/${taskId}`) {
                ...
                if (method === "PATCH") {
                    ...
                    if (typeof body.status === "string")
                        updateWorkItemStatus(db, taskId, body.status);
            ...
            if (url === "/api/tasks" && method === "GET") {
                ...
                const items = listWorkItems(db, { status: q.status, source: q.source });
            ...
            if (url === "/api/tasks" && method === "POST") {
                ...
                if (!body || typeof body.title !== "string") {
                    sendJson(res, 400, { ok: false, error: "body.title is required" });
                    return;
                }
            ...
            if (taskId && subAction === "plan" && !subSub && method === "PUT") {
                ...
                if (!body || !Array.isArray(body.steps)) {
                    sendJson(res, 400, { ok: false, error: "body.steps must be an array" });
                    return;
                }
                const normSteps = body.steps.map((s) => {
                    if (typeof s === "string")
                        return s;
                ...
                const steps = replaceTaskPlan(db, taskId, normSteps);
            ...
            if (taskId && subAction === "patch" && !subSub && method === "POST") {
                ...
                const steps = listTaskPlanSteps(db, taskId);
                if (steps.length > 0) {
                    const chars = { todo: "[ ]", doing: "[~]", done: "[x]", skipped: "[-]", failed: "[!]" };
                ...
                if (result.ok) {
                    ...
                    if (apply)
                        updateWorkItemStatus(db, taskId, "done");
                    else if (data?.changeSetId)
                        updateWorkItemStatus(db, taskId, "needs_review");
                }
                else {
            ...
            if (parts.length === 3 && parts[0] === "api" && parts[1] === "plan-steps" && method === "PATCH") {
                ...
                if (typeof body.status === "string")
                    updateTaskPlanStepStatus(db, stepId, body.status);
                if (typeof body.notes === "string")
                    updateTaskPlanStepNotes(db, stepId, body.notes);
                if (typeof body.title === "string")
                    updateTaskPlanStepTitle(db, stepId, body.title);
                if (typeof body.sortOrder === "number")
                    reorderTaskPlanStep(db, stepId, body.sortOrder);
            ...
            if (url === "/api/chat/sessions" && method === "POST") {
                const b = await readJson(req);
                const s = createChatSession(db, { title: typeof b?.title === "string" ? b.title : undefined, scope: typeof b?.scope === "string" ? b.scope : undefined, taskId: typeof b?.taskId === "string" ? b.taskId : undefined, changeSetId: typeof b?.changeSetId === "string" ? b.changeSetId : undefined });
            ...
            if (parts.length === 4 && parts[0] === "api" && parts[1] === "chat" && parts[2] === "sessions") {
                ...
                if (method === "POST") {
                    ...
                    const result = await sendChatMessage({ root, config, db, events, indexer, sessionId: id, prompt: b.prompt, actionKind: typeof b.actionKind === "string" ? b.actionKind : undefined, model: typeof b.model === "string" ? b.model : undefined });
            ...
            if (url === "/api/chat/messages" && method === "POST") {
                ...
                const result = await sendChatMessage({ root, config, db, events, indexer, sessionId: typeof b.sessionId === "string" ? b.sessionId : undefined, prompt: b.prompt, actionKind: typeof b.actionKind === "string" ? b.actionKind : undefined, model: typeof b.model === "string" ? b.model : undefined });
                sendJson(res, 200, { ok: result.ok, session: result.session, userMessage: result.userMessage, assistantMessage: result.assistantMessage, result: result.result });
                return;
            }
            ...
            if (url === "/api/changes" && method === "GET") {
                const q = getQuery(rawUrl);
                const items = listChangeSets(db, { status: q.status, taskId: q.taskId });
            ...
            if (changeId && !changeSub && method === "GET") {
                ...
                if (!cs) {
                    sendJson(res, 404, { ok: false, error: "Change set not found" });
                    return;
                }
            ...
            if (changeId && changeSub === "revert" && method === "POST") {
                ...
                if (result.ok)
                    updateChangeSetStatus(db, changeId, "reverted");
                sendJson(res, result.ok ? 200 : 400, result);
                return;
            }
            ...
            if (changeId && changeSub === "file-status" && method === "PATCH") {
                ...
                const review = updateChangedFileStatus(db, changeId, body.path, body.status);
                if (!review) {
                    sendJson(res, 404, { ok: false, error: "File not found in change set" });
                    return;
                }
            ...
            if (parts.length === 3 && parts[0] === "api" && parts[1] === "hunks" && method === "PATCH") {
                ...
                if (!body || typeof body.status !== "string") {
                    sendJson(res, 400, { ok: false, error: "body.status is required" });
                    return;
                }
                const hunk = updateChangedHunkStatus(db, parts[2], body.status);
                if (!hunk) {
                    sendJson(res, 404, { ok: false, error: "Hunk not found" });
                    return;
                }
            ...
            if (url === "/api/dashboard" && method === "GET") {
                ...
                for (const r of impRows)
                    impMap.set(r.id, Math.round(r.inbound * 3 + r.outbound * 1 + r.symbols * 0.5));
                const allTags = db.prepare("SELECT ft.file_id, t.name FROM file_tags ft JOIN tags t ON t.id=ft.tag_id").all();
                const tagMap = new Map();
                for (const r of allTags) {
                    const arr = tagMap.get(r.file_id) ?? [];
                    arr.push(r.name);
                    tagMap.set(r.file_id, arr);
                }
