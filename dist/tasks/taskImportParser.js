export function parseTaskImport(text, format) {
    const raw = text.trim();
    if (!raw)
        return { format: "plain", tasks: [], warnings: ["Empty input"] };
    const detected = format ?? detectFormat(raw);
    const warnings = [];
    let tasks;
    switch (detected) {
        case "json":
            tasks = parseJson(raw, warnings);
            break;
        case "csv":
            tasks = parseCsv(raw, warnings);
            break;
        case "markdown":
            tasks = parseMarkdown(raw, warnings);
            break;
        default:
            tasks = parsePlain(raw, warnings);
            break;
    }
...
function parsePlain(text, warnings) {
    const tasks = [];
...
function parseMarkdown(text, warnings) {
    const tasks = [];
    let currentBucket;
    for (const raw of text.split("\n")) {
        let line = raw.trim();
        ...
        if (cbMatch) {
            ...
            if (checked)
                warnings.push(`Task "${title.slice(0, 50)}" marked as completed`);
            continue;
        }
        // Bullet items
...
function parseCsv(text, warnings) {
    ...
    if (rows.length < 2) {
        warnings.push("CSV must have header + data rows");
        return [];
    }
...
function parseJson(text, warnings) {
    ...
    if (Array.isArray(parsed)) {
        return parsed.map((item) => jsonToTask(item, warnings)).filter(Boolean);
    }
    if (typeof parsed === "object" && parsed) {
        const obj = parsed;
        if (Array.isArray(obj.tasks)) {
            return obj.tasks.map((item) => jsonToTask(item, warnings)).filter(Boolean);
        }
    }
    warnings.push("JSON must be an array or {tasks:[...]}");
    return [];
}
function jsonToTask(item, warnings) {
    if (typeof item !== "object" || !item)
        return null;
    const t = item;
    const title = typeof t.title === "string" ? t.title.trim() : "";
    if (!title) {
        warnings.push("JSON item missing title");
        return null;
    }
    return {
        title,
        ...
        checks: Array.isArray(t.checks) ? t.checks : undefined,
    };
}
