...
export function importLocalTasks(args) {
    ...
    const result = { created: 0, skipped: 0, bucketsCreated: 0, taskIds: [], warnings: [...preview.warnings] };
...
export function importOrganizerTasks(args) {
    ...
    for (const b of buckets)
        existingBuckets.set(b.name.toLowerCase(), b.id);
    function resolveBucketId(taskBucket) {
        const name = taskBucket ?? defaults.bucketName ?? null;
        if (!name)
            return defaults.bucketId ?? null;
        const lower = name.toLowerCase();
        if (existingBuckets.has(lower))
            return existingBuckets.get(lower);
