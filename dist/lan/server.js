...
export function startLanServer(args) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (pathname === "/api/state" && req.method === "GET") {
            void (async () => {
                const state = { ...lanState, uptime: lanState.uptime };
                try {
                    const store = getApprovalStore(root);
                ...
                try {
                    ...
                    try {
                        if (kind === "patch" || kind === "build") {
                            const outcome = await runPatch({ db, root, config, events, indexer }, text, { apply: true, review: true, signal: currentAbort.signal, modelOverride: selectedModel });
                            const rendered = renderAgentResult({
                                ...
                                evidence: {
                                    ...
                                    checksRun: outcome.checks.map((c) => ({ name: c.name ?? "", ok: c.ok ?? false })),
                                ...
                                checksRun: outcome.checks.map((c) => c.name ?? ""),
                            });
                            lanState.lastAgentResult = { status: outcome.ok ? "completed" : "failed", text: rendered };
                            onAnswer(rendered);
                        }
                        else {
                            const result = await runAsk({ db, root, config, events, indexer }, text, { signal: currentAbort.signal, modelOverride: selectedModel });
        ...
        if ((pathname === "/api/approvals/approve-all" || pathname === "/api/approvals/reject-all") && req.method === "POST") {
            void (async () => {
                try {
                    ...
                    let count = 0;
                    if (isApprove)
                        count = await store.approveAll();
                    else
                        count = await store.rejectAll();
