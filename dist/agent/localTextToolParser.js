...
function stripCodeFences(text) {
    ...
    let changed = false;
    let out = text.replace(fence, (_m, body) => {
...
function coerceToolObject(obj) {
    ...
    if (!args) {
        ...
        for (const [k, v] of Object.entries(obj)) {
            if (k === "tool" || k === "name" || k === "function" || k === "args" || k === "arguments" || k === "input" || k === "type")
                continue;
            top[k] = v;
        }
        args = top;
...
function parseToolInner(inner, raw, repaired, warnings) {
    ...
    try {
        ...
        if (!coerced)
            return { kind: "invalid", error: "Tool call missing 'tool' field.", raw };
...
export function parseLocalTextActions(text) {
    if (!text || !text.trim())
        return [];
        ...
        if (balanced) {
            ...
            if (before.length <= 40) {
                try {
                    ...
                    const coerced = coerceToolObject(parsed);
                    if (coerced)
                        return [{ kind: "tool_call", tool: coerced.tool, args: coerced.args, raw: text, repaired: true, warnings }];
                }
