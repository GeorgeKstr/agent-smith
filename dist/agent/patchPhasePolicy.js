export function checkPatchPhasePolicy(input) {
    const { phase, toolName, args, progress, maxTotalSearchesPerRun, maxSearchesAfterFirstRead, requireReasonForSearchAfterRead } = input;
    if (toolName !== "search") {
        return { allowed: true };
    }
    if (progress.totalSearches >= maxTotalSearchesPerRun) {
        return {
            allowed: false,
            summary: "Search limit reached. Patch mode cannot keep searching without making a change.",
            nextActions: [
    ...
    if (phase === "ready_to_change" || phase === "ready_to_propose" || phase === "changed" || phase === "verified") {
        ...
        if (progress.searchesAfterFirstRead >= maxSearchesAfterFirstRead) {
            return {
                allowed: false,
                summary: "Search after read limit reached. You already inspected a file and must now make progress.",
                nextActions: [
        ...
        if (requireReasonForSearchAfterRead && !reason) {
            return {
                allowed: false,
                summary: "Search after reading a file requires a specific reason in the 'reason' field.",
                nextActions: [
        ...
        if (isBroadSearchQuery(query)) {
            return {
                allowed: false,
                summary: `Search query "${query}" is too broad after a file has already been read.`,
                nextActions: [
...
export function isBroadSearchQuery(query) {
