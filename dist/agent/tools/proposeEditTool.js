export const proposeEditTool = {
    name: "propose_edit",
    description: "Propose a concrete edit after inspecting files. Use this when you know what should change but want validation before calling edit. Does NOT modify files.",
    parameters: {
        type: "object",
        properties: {
            path: {
    ...
    async handler(rawArgs, ctx) {
        ...
        const wasRead = ctx.memory?.filesRead?.some((f) => f.path === relPath);
        if (!wasRead) {
            return {
                ok: false,
