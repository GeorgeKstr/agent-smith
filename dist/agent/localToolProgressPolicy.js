export function checkLocalToolProgressPolicy(input) {
    ...
    if (toolName === "search" && progress.consecutiveSearches >= input.maxConsecutiveSearches) {
        return {
            allowed: false,
            summary: "Search limit reached. You have already searched repeatedly without reading any result.",
            nextActions: [
    ...
    if (toolName === "read" && progress.consecutiveReads >= input.maxConsecutiveReads) {
        return {
            allowed: false,
            summary: "Read limit reached. You have already read repeatedly without making progress.",
            nextActions: [
