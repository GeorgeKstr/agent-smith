...
import { generateWithProvider, isProviderAvailable, listProviderModels, resolveProviderModelName } from "../providers/providers.js";
...
import { runQwenFunctionLoop } from "./tools/qwenTools.js";
import { createLocalProjectTools } from "./tools/localTools.js";
...
export async function runPatch(deps, task, options = { apply: true }) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (options.review) {
                try {
                    const cp = await createGitCheckpoint({
                        root,
                        db,
                        label: `before proposed patch: ${task.slice(0, 80)}`
                    });
                    ...
                    return {
                        ...
                        files,
                        checks: [],
                        message: `Patch stored as proposed change set: ${cs.id}`,
                        changeSetId: cs.id,
                        checkpointId: cp.id
                    };
                }
                    ...
                    return {
                        ...
                        files,
                        checks: [],
                        message: `Failed to create change set: ${err instanceof Error ? err.message : String(err)}`
                    };
                }
            }
            finishTask(db, taskId, true);
        ...
        for (const file of files)
            recordEdit(db, taskId, file, diff);
...
function repairPrompt(basePrompt, previousDiff, error) {
    return `${basePrompt}
