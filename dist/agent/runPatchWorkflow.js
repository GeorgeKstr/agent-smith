...
import { buildExplorationResult, renderExplorationForPlan, renderExplorationForApply, emptyExplorationResult, } from "./artifacts/explorationResult.js";
...
import { shouldExitExplore, shouldExitPlanPatch, shouldExitExploreForTask, } from "./phaseTransitions.js";
...
export async function runPatchWorkflow(input) {
    const { packet, leads, tools, provider, model, runtimeIntent, config, root, db, events, projectRules, fileCards, taskId, } = input;
    const changedFiles = [];
    const checksRun = [];
    ...
    function resolvePhaseModel(phase) {
    ...
    const sharedInputBase = {
        ...
        runtimeIntent, tools, mode: "patch",
    };
    // ═══════════════════════════════════════════════════════════════════
    ...
    if (input.taskKind === "ui_style_patch") {
        ...
        for (const f of styleAppResult.filesEdited)
            changedFiles.push(f);
        for (const f of styleAppResult.filesCreated)
            changedFiles.push(f);
        ...
        return {
            ...
            status: styleAppResult.changed ? "completed" : "failed",
            finalText,
    ...
    for (const f of applicationResult.filesEdited)
        changedFiles.push(f);
    for (const f of applicationResult.filesCreated)
        changedFiles.push(f);
...
// ═══════════════════════════════════════════════════════════════════
async function runGreenfieldApply(input, sharedInputBase, changedFiles, checksRun, metrics, overallEvidence, memory, providers, leads) {
    ...
    return {
        ...
        status: applicationResult.changed ? "completed" : "failed",
        finalText,
        changedFiles: uniqueChanged,
        checksRun: uniqueChecks,
        metrics,
...
function mergeMetricsInto(target, source) {
...
function mergeEvidenceInto(target, source) {
    for (const f of source.filesRead) {
        if (!target.filesRead.includes(f))
            target.filesRead.push(f);
    }
    for (const f of source.filesEdited) {
        if (!target.filesEdited.includes(f))
            target.filesEdited.push(f);
    }
    for (const f of source.filesCreated) {
        if (!target.filesCreated.includes(f))
            target.filesCreated.push(f);
    }
