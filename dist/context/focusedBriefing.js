...
Confidence: ${tp.confidence}`);
    ...
    if (input.fileCards && input.fileCards.length > 0) {
        const cardText = input.fileCards.slice(0, 8).map((c) => `${c.path}: ${c.purpose} | Exports: ${c.exports.slice(0, 4).join(", ") || "none"}`).join("\n");
    ...
    const leadsText = renderRetrievalLeads(leads, tokenBudget.maxPromptTokens <= 4096 ? 160 : 240);
